...
$(document).ready(function () {
    ...
    var data = {'get_questions_by_level': '1', 'level': window.level};
    if (window.sub_id != '0') {
        ...
        swal({
            ...
            if (parseInt(user_id) != 0) {
                if (window.ratio >= PASSING_PER) {
                    ...
                    var getOldlevel = fetch_level_data(user_id, window.category_id, window.sub_id);
                    var OldLevel = (getOldlevel['error'] == 'false') ? getOldlevel['data']['level'] : 1;

                    if (parseInt(window.level) == parseInt(OldLevel)) {
                        //update level data
                        var uplevel = (parseInt(1) + parseInt(window.level));
                        set_level_data(user_id, window.category_id, window.sub_id, uplevel);
                        ...
                        var data = {'set_users_statistics': '1', 'user_id': user_id, 'questions_answered': window.questions_answered, 'correct_answers': window.correct, 'category_id': window.category_id, 'ratio': window.ratio, 'coins': USER_COIN};
                        var result = get_apis_data(data);                        
                    }
                }
            }
            window.bookmarked_list = JSON.parse(localStorage.getItem("review"));
            $.redirect('resulting.php', {'bookmarked_list': window.bookmarked_list, 'get_coin': GET_COIN, 'users_coins': USER_COIN, 'total_question': window.total_question_percentage, 'currect_ans': window.correct, 'wrong_ans': window.incorrect, 'user_id': user_id, 'ratio': window.ratio, 'score': window.points, 'level': window.level, 'max_level': max_level, 'subcategory': window.sub_id, 'category': window.category_id, 'subcategory_name': window.sub_name, 'type': 'playquiz'});
        });
    } else {
        $('#boockmark_question').removeClass('active');
        $(".progress").empty();
...
function startTimer(timePassed1) {
    var timePassed = (timePassed1 === 0) ? 0 : (TIME_LIMIT - timePassed1);
