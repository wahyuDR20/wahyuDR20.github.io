...
$(document).ready(function () {
    ...
    var data = {'get_questions_by_contest': '1', 'contest_id': contest_id};
    ...
    $("input[type='radio']").click(function () {
        ...
        if (radioValue == res[window.count]['answer'].toLowerCase()) {
            ...
            window.points = parseInt(window.points) + parseInt(get_point(timeLeft));
...
function loadQuestions(res) {
    ...
    if (window.attempt >= res.length) {
        swal({
            ...
            if (user_id) {
                var data = {'contest_update_score': '1', 'user_id': user_id, 'contest_id': contest_id, 'questions_attended': window.questions_answered, 'correct_answers': window.correct, 'score': window.points};
                var result = get_apis_data(data);
            }
            window.bookmarked_list = JSON.parse(localStorage.getItem("review"));
            $.redirect('contest-result.php', {'bookmarked_list': window.bookmarked_list, 'users_coins': window.coins, 'total_question': window.total_question_percentage, 'currect_ans': window.correct, 'wrong_ans': window.incorrect, 'user_id': user_id, 'ratio': window.ratio, 'score': window.points});
        });
    } else {
