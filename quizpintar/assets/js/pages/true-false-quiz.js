        ...
        swal({
            ...
            if (parseInt(user_id) != 0) {
                if (window.ratio >= PASSING_PER) {
                    var res = {'set_monthly_leaderboard': '1', 'user_id': user_id, 'score': window.points};
                    get_apis_data(res);
                }
                var res = {'set_users_statistics': '1', 'user_id': user_id, 'questions_answered': window.questions_answered, 'correct_answers': window.correct, 'category_id': '0', 'ratio': window.ratio, 'coins': USER_COIN};
                get_apis_data(res);

            }
            window.bookmarked_list = JSON.parse(localStorage.getItem("review"));
            $.redirect('resulting.php', {'bookmarked_list': window.bookmarked_list, 'get_coin': GET_COIN, 'users_coins': USER_COIN, 'total_question': window.total_question_percentage, 'currect_ans': window.correct, 'wrong_ans': window.incorrect, 'user_id': user_id, 'ratio': window.ratio, 'score': window.points, 'level': window.level, 'subcategory': window.sub_id, 'category': window.category_id, 'subcategory_name': window.sub_name, 'type': 'truefalsequiz'});
        });
    } else {
